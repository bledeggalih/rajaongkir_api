...
import 'package:rajaongkir_api/models/subdistrict_model.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  _getSubdistrict(String type, int cityId) async {
    setState(() {
      _loadSubs = true;
    });
    type == "origin" ? _originSubdistrictLists.clear() : _destinationSubdistrictLists.clear();
    final response = await API().getSubdistrict(cityId);
    if(response.statusCode == 200){
      final status = json.decode(response.body)['rajaongkir']['status'];
      if (status!['code'] == 200 && status!['description'] == "OK") {
        final data = json.decode(response.body);
        setState(() {
          for (Map<String, dynamic> i in data!['rajaongkir']['results']) {
            ...
            type == "origin" ? _originSubdistrictLists.add(
                SubdistrictModel.fromJson(i)) : _destinationSubdistrictLists
                .add(SubdistrictModel.fromJson(i));
          }
        });
      }else{
        _showMsg("${status!['code']}, ${status!['description']}");
      ...
      _showMsg("Error ${response.statusCode} while get Subdistrict data!");
    }
    setState(() {
      _loadSubs = false;
    });
  ...
  _countShippingCost(int org, String orgTp, int des, String desTp, double weight, String courier) async {
    setState((){

    });
    _courierServiceLists.clear();
    final response = await API().getEstimatedCost(org, orgTp, des, desTp, weight, courier);
    if(response.statusCode == 200){
                ...
                children: [
                            ...
                            child: DropdownButton<ProvinceModel>(
                              ...
                              onChanged: (ProvinceModel? orgProvValue) {
                                ...
                                setState((){
                                  ...
                                  _originSubdistrictLists.clear();
                                  _originSubdistrictAddress = null;
                                });
                              },
                            ),
                          ),
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<CityModel>(
                              ...
                              onChanged: (CityModel? orgCityValue) {
                                FocusScope.of(context).requestFocus(FocusNode());
                                _getSubdistrict('origin',int.parse(orgCityValue!.cityId));
                                setState(() {
                  ...
                  Container(
                      ...
                      child: ButtonTheme(
                        ...
                        child: InputDecorator(
                          decoration: InputDecoration(
                            contentPadding: const EdgeInsets.only(left: 10, bottom: -5),
                            label: const Text("Subdistrict"),
                            border: OutlineInputBorder(
                                borderRadius: BorderRadius.circular(5.0)),
                          ),
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<SubdistrictModel>(
                              items: _originSubdistrictLists.map<DropdownMenuItem<SubdistrictModel>>((SubdistrictModel val) {
                              ...
                              isExpanded: true,
                              hint: const Text("Subdistrict"),
                              icon: const Icon(Icons.arrow_drop_down),
                              value: _originSubdistrictAddress,
                              onChanged: (SubdistrictModel? orgSubdValue) {
                                FocusScope.of(context).requestFocus(FocusNode());
                                setState(() {
                                  _originSubdistrictAddress = orgSubdValue;
                                  _originAddress = "${_originSubdistrictAddress!.subdistrictName}, ${_originSubdistrictAddress!.type} ${_originSubdistrictAddress!.city}, ${_originSubdistrictAddress!.province}";
                                });
                                if (kDebugMode) {
                                  print("Address Origin = ?prov=${_originProvinceAddress!.provinceId}&city=${_originCityAddress!.cityId}&district=${_originSubdistrictAddress!.subdistrictId}");
                                }
                              },
                            ),
                          ),
                            ...
                            child: DropdownButton<ProvinceModel>(
                              ...
                              onChanged: (ProvinceModel? desProvValue) {
                                ...
                                setState((){
                                  ...
                                  _destinationSubdistrictLists.clear();
                                  _destinationSubdistrictAddress = null;
                                });
                              },
                            ),
                          ),
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<CityModel>(
                              ...
                              onChanged: (CityModel? desCityValue) {
                                FocusScope.of(context).requestFocus(FocusNode());
                                _getSubdistrict('destination',int.parse(desCityValue!.cityId));
                                setState(() {
                  ...
                  Container(
                      ...
                      child: ButtonTheme(
                        ...
                        child: InputDecorator(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<SubdistrictModel>(
                              items: _destinationSubdistrictLists.map<DropdownMenuItem<SubdistrictModel>>((SubdistrictModel val) {
                                return DropdownMenuItem<SubdistrictModel>(
                                  value: val,
                                  child: Text(
                                    val.subdistrictName,
                              ...
                              hint: const Text("Subdistrict"),
                              icon: const Icon(Icons.arrow_drop_down),
                              value: _destinationSubdistrictAddress,
                              onChanged: (SubdistrictModel? desSubdValue) {
                                FocusScope.of(context).requestFocus(FocusNode());
                                setState(() {
                                  _destinationSubdistrictAddress = desSubdValue;
                                  _destinationAddress = "${_destinationSubdistrictAddress!.subdistrictName}, ${_destinationSubdistrictAddress!.type} ${_destinationSubdistrictAddress!.city}, ${_destinationSubdistrictAddress!.province}";
                                });
                                if (kDebugMode) {
                                  print("Address Destination = ?prov=${_destinationProvinceAddress!.provinceId}&city=${_destinationCityAddress!.cityId}&district=${_destinationSubdistrictAddress!.subdistrictId}");
                                }
                              },
                            ),
                          ),
                  ...
                  Center(
                    child: ElevatedButton(
                        onPressed: () async {
                          Map data = {
                            "orId" : _originSubdistrictAddress!.subdistrictId,
                            "orTp" : "subdistrict",
                            "desId" : _destinationSubdistrictAddress!.subdistrictId,
                            "desTp" : "subdistrict",
                            "weight" : _weight.text,
                            "courier" : _choosedCourier!.code
                          };
                          _countShippingCost(int.parse(data['orId']), data['orTp'], int.parse(data['desId']), data['desTp'], double.parse(data['weight']), data['courier']);
